...
export async function createPage(formData: FormData) {
  ...
  let id = "";
  try {
    const [created] = await db.insert(landingPages).values({ brandId, campaignId, name, slug, conversionGoal: "subscription", draftContent: json(document), draftSeo: json(seo), createdBy: actor.id, updatedBy: actor.id }).returning({ id: landingPages.id });
...
export async function savePageDraft(input: SavePageDraftInput): Promise<SavePageDraftResult> {
  ...
  const { db, client } = getDatabase();
  try {
    const [saved] = await db.update(landingPages).set({ name, slug, brandId: input.brandId, campaignId: input.campaignId || null, conversionGoal: conversionGoal || null, draftContent: json(parsed.data), draftRevision: input.expectedRevision + 1, updatedAt: new Date(), updatedBy: actor.id })
...
async function availableCopySlug(brandId: string, baseSlug: string) {
  ...
  try {
    for (let index = 1; index <= 50; index += 1) {
      ...
      const [existing] = await db.select({ id: landingPages.id }).from(landingPages).where(and(eq(landingPages.brandId, brandId), eq(landingPages.slug, slug))).limit(1);
...
export async function duplicatePage(input: { sourceId: string; targetBrandId: string; targetCampaignId?: string | null }) {
  ...
  const { db, client } = getDatabase();
  let source: { name: string; slug: string; brandId: string; conversionGoal: string | null; draftContent: unknown; draftSeo: unknown } | undefined;
  try {
    [source] = await db.select({ name: landingPages.name, slug: landingPages.slug, brandId: landingPages.brandId, conversionGoal: landingPages.conversionGoal, draftContent: landingPages.draftContent, draftSeo: landingPages.draftSeo }).from(landingPages).where(eq(landingPages.id, input.sourceId)).limit(1);
  } finally { await client.end(); }
  ...
  const sameBrand = source.brandId === input.targetBrandId;
  const document = sameBrand ? baseDocument : clearAssetReferences(baseDocument);
  const seoParsed = pageSeoSchema.safeParse(source.draftSeo);
  const seo = seoParsed.success ? { ...seoParsed.data, canonicalUrl: null, socialAssetId: sameBrand ? seoParsed.data.socialAssetId : null } : defaultPageSeo(`${source.name} Copy`);
  const slug = await availableCopySlug(input.targetBrandId, source.slug);
  const connection = getDatabase();
  let createdId = "";
  try {
    const [created] = await connection.db.insert(landingPages).values({ brandId: input.targetBrandId, campaignId: input.targetCampaignId || null, name: `${source.name} Copy`.slice(0, 160), slug, conversionGoal: source.conversionGoal, draftContent: json(document), draftSeo: json(seo), createdBy: actor.id, updatedBy: actor.id }).returning({ id: landingPages.id });
