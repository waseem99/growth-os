...
export default async function PagesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  try {
    ...
    return <main className="shell compact-shell">
      <section className="section-heading"><p className="eyebrow">Landing pages</p><h1>Build, reuse and iterate campaign pages.</h1><p>Structured drafts only. Preview and publishing use the same page engine as the public renderer.</p></section>
      {canManage && <form className={styles.create} action={createPage}>
        <input name="name" required placeholder="Page name" maxLength={160} /><input name="slug" placeholder="URL slug (optional)" maxLength={120} />
        <select name="brandId" required defaultValue=""><option value="" disabled>Brand</option>{brandRows.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select>
