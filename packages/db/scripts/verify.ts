...
const requiredTables = ["app_users","brands","domains","offers","offer_versions","campaigns","templates","landing_pages","page_versions","page_publications","assets","asset_usages","experiments","variants","analytics_events","conversions","integrations","audit_logs","ai_jobs","rate_limit_buckets"];
try {
  ...
  for (const version of ["0002_page_draft_seo", "0003_tracking_context", "0004_experiment_integrity", "0005_operability"]) if (!migrations.some((row) => row.version === version)) throw new Error(`Expected migration ${version} is not recorded`);
  ...
  const operabilityIndexes = await sql<{ indexname: string }[]>`SELECT indexname FROM pg_indexes WHERE schemaname='public' AND indexname IN ('rate_limit_buckets_bucket_idx','audit_logs_correlation_idx')`;
  if (operabilityIndexes.length !== 2) throw new Error("Operability indexes are incomplete");

  const [experiment] = await sql<{ status: string; variants: string; allocation: string; controls: string }[]>`SELECT e.status, count(v.id)::text AS variants, coalesce(sum(v.allocation),0)::text AS allocation, count(*) FILTER (WHERE v.is_control)::text AS controls FROM experiments e JOIN variants v ON v.experiment_id=e.id WHERE e.id='00000000-0000-4000-8000-000000000900' GROUP BY e.id, e.status`;
  ...
  const [seedEvents] = await sql<{ landing_views: string; sessions: string; cta_clicks: string; signup_starts: string; checkout_starts: string; control_variant_events: string }[]>`
    SELECT count(*) FILTER (WHERE event_name='landing_view')::text AS landing_views, count(DISTINCT session_id) FILTER (WHERE event_name='landing_view')::text AS sessions, count(*) FILTER (WHERE event_name='cta_click')::text AS cta_clicks, count(*) FILTER (WHERE event_name='signup_start')::text AS signup_starts, count(*) FILTER (WHERE event_name='checkout_start')::text AS checkout_starts, count(*) FILTER (WHERE variant_id='00000000-0000-4000-8000-000000000901')::text AS control_variant_events
    FROM analytics_events WHERE page_id='00000000-0000-4000-8000-000000000050' AND properties->>'seeded'='true' AND coalesce(properties->>'testTraffic','false') <> 'true'`;
  const [testTraffic] = await sql<{ events: string; variant_id: string | null }[]>`SELECT count(*)::text AS events, min(variant_id::text) AS variant_id FROM analytics_events WHERE page_id='00000000-0000-4000-8000-000000000050' AND properties->>'seeded'='true' AND properties->>'testTraffic'='true'`;
  const [seedConversions] = await sql<{ subscriptions: string; revenue: string; currency: string; variant_id: string | null }[]>`SELECT count(*) FILTER (WHERE event_name='subscription_started')::text AS subscriptions, coalesce(sum(value),0)::text AS revenue, min(currency) AS currency, min(variant_id::text) AS variant_id FROM conversions WHERE page_id='00000000-0000-4000-8000-000000000050' AND properties->>'seeded'='true' AND coalesce(properties->>'testTraffic','false') <> 'true'`;
  ...
  const pageDocuments = await sql<{ slug: string; schema_version: string | null; blocks: string }[]>`SELECT lp.slug, pv.content->>'schemaVersion' AS schema_version, jsonb_array_length(pv.content->'blocks')::text AS blocks FROM page_publications pp JOIN landing_pages lp ON lp.id=pp.page_id JOIN page_versions pv ON pv.id=pp.version_id WHERE lp.id IN ('00000000-0000-4000-8000-000000000050','00000000-0000-4000-8000-000000000150') ORDER BY lp.slug`;
  ...
  const [first] = await sql<{ count: number }[]>`INSERT INTO rate_limit_buckets (key,bucket,count) VALUES ('verify:release',${bucket},1) ON CONFLICT (key,bucket) DO UPDATE SET count=rate_limit_buckets.count+1 RETURNING count`;
  const [second] = await sql<{ count: number }[]>`INSERT INTO rate_limit_buckets (key,bucket,count) VALUES ('verify:release',${bucket},1) ON CONFLICT (key,bucket) DO UPDATE SET count=rate_limit_buckets.count+1 RETURNING count`;
  if (first?.count !== 1 || second?.count !== 2) throw new Error("DB-backed rate-limit counter is not atomic");
  await sql`DELETE FROM rate_limit_buckets WHERE key='verify:release'`;

  console.log(`Verified ${requiredTables.length} tables, ordered migrations, operability controls, experiment integrity, test-traffic isolation and reconciled acquisition data`);
