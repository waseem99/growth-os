...
try {
  await sql.begin(async (tx) => {
    ...
    await tx`INSERT INTO campaigns (id, brand_id, offer_version_id, name, platform, objective, status, utm_defaults, created_by, updated_by) VALUES (${ids.skillupCampaign}, ${ids.skillupBrand}, ${ids.skillupOfferVersion}, 'SkillUp Demo Acquisition', 'tiktok', 'subscription', 'draft', ${tx.json({ source: "tiktok", medium: "paid_social" })}, ${ids.user}, ${ids.user}) ON CONFLICT (id) DO NOTHING`;
  ...
  console.log("Seeded GrowthOS SkillUp and Jalwa data");
